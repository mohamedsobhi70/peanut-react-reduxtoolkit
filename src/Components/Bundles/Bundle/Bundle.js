        ...
        <div className="bundle-card">
            ...
            <div className="flex items-start justify-between gap-2">
                <div className="relative text-center">
                    <div>
                        <svg className="absolute" width={((+item.amount - +item.available) / +item.amount * 112).toFixed(1)} height="54" viewBox={`0 0 ${((+item.amount - +item.available) / +item.amount * 112).toFixed(1)} 54`} fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <svg className="absolute" width="112" height="54" viewBox="0 0 112 54" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="flex justify-around items-center relative" style={{ width: "112px", height: "54px" }}>
                        <div className="">
                            <b className="font-semibold text-Dark text-sm">{(+item.amount - +item.available)}</b>
                            <p className="mb-0 font-medium text-Dark text-[10px]">sold</p>
                        </div>
                        <div className="">
                            <p className="mb-0 font-medium text-Dark text-[10px]">out of</p>
                            <b className="font-semibold text-Dark text-sm">{(item.amount)}</b>
                        </div>
                    </div>
                </div>
                {/* add to wish list  */}
            ...
            <div className="flex flex-col items-center gap-4">
                <Link to={`/bundle-details/${item.id}`} className=" h-80 overflow-hidden">
                    <img src={item.img} alt={item.description} className='object-contain h-full w-full' />
                </Link>
                <h3 className='text-Dark font-semibold text-center flex flex-col'>
                    <Link to={`/bundle-details/${item.id}`}>
                        {item.name}
                    </Link>
                </h3>
            </div>
            ...
            <div className="flex items-center gap-3">
                ...
                <p className="text-Primary text-center shrink-0 grow text-lg font-semibold">
                    EGP {item.price}
                </p>
            </div>
        </div>
